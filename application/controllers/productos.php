...
class Productos extends CI_Controller
{
      ...
      public function ExportarExcel($nombre='') {
       if($this->uri->segment(3)!='0') {
        ...
        $result = $this->db->from("productos pr")
                            ...
                            ->join("lineas l","pr.prod_linea_id=l.lin_id","left")
                            ->join("marcas ma","pr.prod_marca_id=ma.mar_id","left")
        ...
        $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('G1', 'CANTIDAD MINIMA')
                ->setCellValue('H1', 'STOCK')
                ->setCellValue('I1', 'CODIGO SUNAT')                
                ->setCellValue('J1', 'P VENTA2')
                ->setCellValue('K1', 'P VENTA3')
                ->setCellValue('L1', 'P VENTA4')
                ->setCellValue('M1', 'P VENTA5')
                ->setCellValue('N1', 'CODIGO DE BARRA')
                ->setCellValue('O1', 'COMISION VENDEDOR')
                ->setCellValue('P1', 'GARANTIA')
                ->setCellValue('Q1', 'DESCUENTO')
                ->setCellValue('R1', 'CADUCIDAD')                
                ->setCellValue('S1', 'LINEA')
                ->setCellValue('T1', 'MARCA')
                ->setCellValue('U1', 'UBICACION')
                ->setCellValue('V1', 'OBSERVACIONES')
                ->setCellValue('W1', 'ALMACÉN');
        ...
        foreach ($result as $value) {
            ...
            $spreadsheet->getActiveSheet()
                        ...
                        ->setCellValue('G'.$i, $value->prod_cantidad_minima)
                        ->setCellValue('H'.$i, $stock)
                        ->setCellValue('I'.$i, $value->prod_codigo_sunat)
                        ...
                        ->setCellValue('S'.$i, $value->lin_nombre)
                        ->setCellValue('T'.$i, $value->mar_nombre)
                         ->setCellValue('U'.$i, $value->prod_ubicacion)
                         ->setCellValue('V'.$i, $value->prod_observaciones)
                         ->setCellValue('W'.$i, $this->session->userdata('almacen_nom'));
            $i++;
        }
   ...
   public function buscar_producto(){
    ...
    public function modal_buscarProducto(){        
        ...
        $data['tipo_comprobante'] = $this->uri->segment(3);
        $this->load->view('productos/modal_buscarProducto',$data);
    }   

    public function modal_buscarProductoHistoriaOtros(){
        $data['tipo_igv'] = $this->tipo_igv_model->select('', '', '', 0);
        $data['configuracion'] = $this->db->from('comprobantes_ventas')->get()->row();   
        $data['tipo_comprobante'] = $this->uri->segment(3);
        $this->load->view('productos/modal_buscarProductoHistoriaOtros',$data);
    }

    public function modal_buscarProductoHistoria(){
        $data['tipo_igv'] = $this->tipo_igv_model->select('', '', '', 0);
        $data['configuracion'] = $this->db->from('comprobantes_ventas')->get()->row();   
        $data['tipo_comprobante'] = $this->uri->segment(3);
        $this->load->view('productos/modal_buscarProductoHistoria',$data);
    }

    public function modal_buscarProductoNota(){
        $data['tipo_igv'] = $this->tipo_igv_model->select('', '', '', 0);
        $data['configuracion'] = $this->db->from('comprobantes_ventas')->get()->row();
        $this->load->view('productos/modal_buscarProductoNota',$data);     
    }

    public function modal_buscarProductoProforma(){
        $data['tipo_igv'] = $this->tipo_igv_model->select('', '', '', 0);
        $data['configuracion'] = $this->db->from('comprobantes_ventas')->get()->row();
        $this->load->view('productos/modal_buscarProductoProforma',$data);
    }
    ...
    public function selectAutocomplete(){
        ...
        $almacen  = $this->input->get('almacen');
        echo json_encode($this->productos_model->selectAutocomplete($linea,$almacen));
    }          


   //CODIGO DE BARRAS 10-11-2020
   public function selectPrecioCodBarra() {
        
        $rowPrecioCodBarra = array();        
        $rowPrecioCodBarra = $this->productos_model->selectPrecioCodBarra($_POST['codBarra']);            
                
        $array = array(            
                 'prod_id' => $rowPrecioCodBarra->prod_id,
                 'prod_nombre' => $rowPrecioCodBarra->prod_nombre,
                 'prod_precio_publico' => $rowPrecioCodBarra->prod_precio_publico,
                 'prod_stock' => $rowPrecioCodBarra->prod_stock,
                 'prod_medida'=> $rowPrecioCodBarra->medida_nombre
                );        
